...
def plot_heat(heat_source:np.array, X:np.array, Y:np.array, Phi:np.array) -> None:
    ...
    def _align(arr, name: str):
        if arr.shape == X.shape:
            return arr
        if arr.T.shape == X.shape:
            return arr.T
        if arr.size == X.size:
            return arr.reshape(X.shape)
        raise ValueError(f"{name} cannot be aligned to grid shape {X.shape}, got {arr.shape}")

    heat_aligned = _align(heat_source, 'heat_source')
    phi_aligned = _align(Phi, 'Phi')

    heat = ax1.pcolormesh(X, Y, heat_aligned, cmap = plt.get_cmap('copper'), shading='nearest')
    ax1.axis('equal')
    ...
    temp = ax2.pcolormesh(X, Y, phi_aligned, cmap = plt.get_cmap('hot'), shading='nearest')
